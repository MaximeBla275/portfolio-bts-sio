<template>
  <PageWrapper>
    ...
    <!-- Formation -->
    <div class="mt-24 block-inv p-8 md:p-12">
      <h2 class="font-display text-3xl font-bold mb-8">Parcours scolaire</h2>
      <div class="space-y-0">
        <div
          v-for="item in formation"
          :key="item.title"
          class="border-t border-[var(--block-border)] py-5 grid grid-cols-1 md:grid-cols-4 gap-3"
        >
          <span class="font-mono text-xs text-[var(--block-muted)] uppercase tracking-widest">{{ item.year }}</span>
          <div class="md:col-span-2">
            <p class="font-mono text-sm font-medium">{{ item.title }}</p>
            <p class="font-mono text-xs text-[var(--block-muted)] mt-1">{{ item.etablissement }}</p>
          </div>
          <span v-if="item.mention" class="self-start text-xs font-mono px-2 py-1 border border-[var(--block-border)] text-[var(--block-muted)] w-fit">
            {{ item.mention }}
          </span>
        </div>
      </div>

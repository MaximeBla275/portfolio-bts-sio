    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-10">
      ...
      <div class="md:col-span-2 space-y-8">
        <div>
          ...
          <p class="font-mono text-base">{{ stage.periode }}</p>
        </div>
        <div>
          <p class="font-mono text-xs uppercase tracking-widest text-[var(--muted)] mb-4">Description</p>
          <p class="font-mono text-base text-[var(--muted)] leading-relaxed">{{ stage.description }}</p>
        </div>
        <div>
          <p class="font-mono text-xs uppercase tracking-widest text-[var(--muted)] mb-4">Missions réalisées</p>
          <ul class="space-y-3">
            <li v-for="(mission, i) in stage.missions" :key="i" class="flex gap-3 font-mono text-base">
          ...
          <div class="space-y-3">
            <NuxtLink v-for="proj in relatedProjects" :key="proj.id" to="/projets"
              ...
              <div class="flex items-center gap-3">
                ...
                <div>
                  <p class="font-mono text-base font-medium group-hover:text-[var(--accent)] transition-colors">{{ proj.title }}</p>
                  <p class="font-mono text-sm text-[var(--muted)]">{{ proj.subtitle }}</p>
                </div>
              </div>
        ...
        <div class="border-l-2 border-[var(--accent)] pl-5">
          ...
          <p class="font-mono text-base text-[var(--muted)] leading-relaxed italic">"{{ stage.bilan }}"</p>
        </div>
      </div>

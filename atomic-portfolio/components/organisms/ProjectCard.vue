<template>
  <NuxtLink
    ...
    class="group block border-b border-[var(--border)] py-6 flex gap-5 px-2 hover:bg-[var(--card)] transition-colors duration-200"
  >
      ...
      <div class="flex items-start justify-between gap-3">
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2 mb-1 flex-wrap">
            ...
            <h3 class="font-display text-lg font-bold group-hover:text-[var(--accent)] transition-colors">{{ project.title }}</h3>
            <BaseTag :variant="project.type">{{ typeLabel }}</BaseTag>
          </div>
          <p class="font-mono text-sm text-[var(--muted)] line-clamp-2 max-w-lg">{{ project.description }}</p>
        </div>
      ...
      <div class="flex flex-wrap items-center gap-3 mt-3">
        ...
        <div class="ml-auto flex items-center gap-3">
          <a
            ...
            download
            class="inline-flex items-center gap-1 font-mono text-xs border border-[var(--accent)] text-[var(--accent)] px-2.5 py-1 hover:bg-[var(--accent)] hover:text-white transition-colors"
            @click.stop
          >
            📄 Rapport
          </a>
          <span class="font-mono text-xs text-[var(--muted)] group-hover:text-[var(--ink)] transition-colors flex items-center gap-1">
            Voir <span class="group-hover:translate-x-0.5 transition-transform inline-block">→</span>
          </span>
        </div>
      </div>
    </div>
